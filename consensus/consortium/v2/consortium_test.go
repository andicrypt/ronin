...
func TestUpgradeAxieProxyCode(t *testing.T) {
	...
	chainConfig := &params.ChainConfig{
		...
		AxieProxyCodeUpgrade: &params.ContractCodeUpgrade{
			CodeAddress: common.Address{0x12},
	...
	for i := 0; i < 5; i++ {
		...
		if header.Number.Uint64() == chainConfig.AaronBlock.Uint64()+1 {
			...
			code := statedb.GetCode(v2.chainConfig.AxieProxyCodeUpgrade.CodeAddress)
			if !bytes.Equal(
				code,
				common.Hex2Bytes(chainConfig.AxieProxyCodeUpgrade.Code),
			) {
